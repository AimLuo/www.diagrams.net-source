...
**注意：** 样式值中不允许使用分号，因为分号用于分隔形状样式中的 ``key=value`` 对。

## 禁用暗色模式颜色

要禁用对单独暗色模式颜色的支持，请在 SVG 元素的 style 属性中添加 ``color-scheme: light;`` 或在单元格样式中使用 ``darkMode=0``。（请注意，Safari 中尚不支持 SVG 图像的暗色模式，它将在亮色和暗色模式下都使用亮色。）
